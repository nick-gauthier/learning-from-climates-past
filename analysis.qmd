...
Note, some papers can have multiple recommendation types. This analysis counts those separately (like we do for periods) so our N is greater than the number of articles. We could do it the other way too (treating papers with multiple rec types as separate categories) if necessary though.
...
Note -- this table suggests there is one row where the recommendation tags are mismatched -- double check!
...
a + b + c
```
Here's another possible visualization, a waffle plot, just for reference.
```{r}
library(waffle)
dat |>
  count(discipline) |>
  ...
  theme_void()
```
And likewise, a treemap.
```{r}
library(treemapify)
dat |>
  count(discipline) |>
  ggplot(aes(area = n, fill = discipline, label = discipline)) +
  geom_treemap() +
...
Again, doing it both ways so we have them for reference.
```{r}
dat |>
  select(period, agw) |>
  unnest(period) |>
...
## Figures for compound questions

### Figure 1: a chart/graph showing answers to questions 1-2. 
...
```
Here's a different way to plot the same data:

```{r fig.width = 10.5, fig.asp = .3}
a <- dat |>
  ggplot(aes(discipline)) +
  geom_bar(aes(fill = agw)) +
    scale_fill_grey(start = 0.8, end = 0.2) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Discipline",
       x = "",
...
### Figure 2: a chart/graph showing answers to questions 3-4. 
...
c <- dat |>
  ggplot(aes(region)) +
  geom_bar(aes(fill = agw)) +
    scale_fill_grey(start = 0.8, end = 0.2, guide = NULL) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Region",
       x = "",
...
### Figure 3: a chart/graph showing answers to questions 5-6. 

Exploring other ways to visualize this:
```{r}
dat |>
  ggplot(aes(rec)) +
  geom_bar(aes(fill = discipline)) +
    scale_fill_brewer(palette = 'Spectral') +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Includes lessons or recommendations",
       x = "",
       y = "Publications",
       fill = "Discipline")
...
dat |>
  ggplot(aes(rec)) +
  geom_bar(aes(fill = discipline), position = 'fill') +
    scale_fill_brewer(palette = 'Spectral') +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Includes lessons or recommendations",
       x = "",
...
dat |>
  ...
  coord_fixed()
```
[Maybe a donut chart would be another option for these?](https://r-charts.com/part-whole/donut-chart/). Just need to avoid pie charts . . .
...
dat |>
  ggplot(aes(rec)) +
  geom_bar(aes(fill = method), position = 'fill') +
    scale_fill_brewer(palette = 'Spectral') +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Includes lessons or recommendations",
       x = "",
...
dat |>
  filter(rec == 'Yes') |>
  count(method) |>
  ggplot(aes(area = n, fill = method, label = method)) +
  geom_treemap(color = 'white') +
    geom_treemap_text(fontface = "italic", colour = "black", place = "centre",
                    grow = FALSE) +
  scale_fill_brewer(palette = 'Spectral', guide = NULL) +
...
### Figure 4: a chart/graph showing answers to question 7.
...
### Figure 5: a chart/ graph showing answers to question 8.
...
Testing different way to look at the data through logistic regression. How well do some of our categories do at predicting whether a paper includes recommendations for the future or not?

This is just a test, but we can explore or expand this in various ways if useful.
